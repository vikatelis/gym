...
class SGDonMNIST(gym.Env):
    def __init__(self):
        ...
        # init placeholder
        self.input_X = tf.placeholder('float32',shape = (None,28,28))
        self.input_y = tf.placeholder('float32',shape = (None))
        self.learning_rate = tf.placeholder(tf.float32, shape=[])
        # init network
        with tf.variable_scope('test_scope'):
            x = tf.layers.flatten(self.input_X)
            logits = tf.layers.dense(x,512,activation = tf.nn.relu)
            logits2 = tf.layers.dense(logits,10,activation = None)
            self.loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels = tf.cast(self.input_y,tf.int32),logits = logits2))
    ...
    def sgd_eval(self, cs_state):
        ...
        for j in range(batches):
            ...
            _, batch_loss = self.sess.run([self.train_step, self.loss], feed_dict={self.input_X: X_b, self.input_y: Y_b, self.learning_rate: lr})
